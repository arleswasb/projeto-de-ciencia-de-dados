...
with tab1:
    ...
    with col_filters_tab1_1:
        ...
        selected_periods_tab1 = st.multiselect(
            'Período(s) da Pandemia:', options=all_periods_tab1, default=all_periods_tab1, key='tab1_periods'
        )
    with col_filters_tab1_2:
        ...
        year_range_tab1 = st.slider(
            'Intervalo de Anos:', min_value=min_year_tab1, max_value=max_year_tab1,
            value=(min_year_tab1, max_year_tab1), key='tab1_years'
        )
    ...
    if not df_chart1.empty:
        fig1 = px.bar(
            df_chart1, x='count', y='genre', color='platform', facet_col='periodo',
            facet_col_wrap=2, title='Gêneros Mais Lançados por Plataforma e Período',
        ...
        # --- AJUSTE MAIS ROBUSTO PARA O EIXO X: Itera sobre todos os eixos X e força independência ---
        for i, axis_id in enumerate(fig1.layout):
            if axis_id.startswith('xaxis'): # Verifica se é um objeto de eixo X
                fig1.layout[axis_id].showticklabels = True
                fig1.layout[axis_id].tickangle = 45 # Girar rótulos para evitar sobreposição
                fig1.layout[axis_id].title.text = 'Número de Lançamentos' # Define o título para cada eixo
                fig1.layout[axis_id].matches = None # Força eixos independentes para exibir todos os rótulos
        # --- FIM DO AJUSTE ---
