...
# Criar as abas
tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8 = st.tabs([ # Adicionado tab8 (nova)
    "Gêneros por Período",
    ...
    "Heatmap de Preços",
    "Tendências de Preços (Linha)" # Nova aba para o gráfico de linha
])
...
with tab1:
    ...
    period_display_order = ['Pré-Pandemia', 'Durante a Pandemia', 'Pós-Pandemia','Período Completo']
        ...
        for i, (plot_df, title, show_legend) in enumerate(plots_to_render):
            with cols[i % num_cols]:
                fig = px.bar(
                    ...
                    orientation='h', height=400, width=300
                )
                fig.update_layout(
            ...
            # Adicionar divisor entre os gráficos, mas não no final
...
# --- TAB 7: Heatmap de Preços por Gênero e Ano (continua como Heatmap) ---
with tab7:
...
# --- NOVA TAB 8: Tendências de Preços (Gráfico de Linha) ---
with tab8:
    st.header("8. Tendências de Preços Médios por Ano (Gráfico de Linha)")
    st.markdown("Analise a evolução dos preços médios de jogos ao longo do tempo, por plataforma ou gênero.")
    ...
    trend_by_option_tab8 = st.radio(
        ...
        ('Plataforma', 'Gênero'),
        key='trend_by_option_tab8_radio'
    )

    # --- LÓGICA DE FILTRAGEM E AGREGAÇÃO PARA O GRÁFICO DE LINHA (COPIADO DA ANTIGA TAB7) ---
    ...
    # Agregação final para o gráfico de linha
    if trend_by_option_tab8 == 'Plataforma':
